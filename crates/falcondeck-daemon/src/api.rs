...
/// Turn requests can carry base64 attachment data. The cross-provider budget
/// allows 15 MB of decoded images plus 25 MB of decoded documents, which
/// expands to about 54 MB on the wire; Axum's 2 MB JSON default would reject
/// valid sends before the daemon can materialize and validate them.
///
/// This is the local ceiling. A relayed send is bounded further by the relay's
/// 40 MiB websocket message limit, which a maximal document turn exceeds —
/// remote clients hit that before this one.
const TURN_REQUEST_BODY_LIMIT_BYTES: usize = 56 << 20;
...
mod tests {
    ...
    #[test]
    fn turn_request_limit_has_headroom_for_the_attachment_budget() {
        // 15 MB of raw images plus 25 MB of raw documents becomes at most this
        // many base64 bytes before the small JSON envelope is added.
        let max_encoded_attachment_bytes = 40_000_000_usize.div_ceil(3) * 4;
        assert!(TURN_REQUEST_BODY_LIMIT_BYTES > max_encoded_attachment_bytes);
    }
