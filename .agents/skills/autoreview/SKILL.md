---
name: autoreview
description: "Run the repo-configured structured closeout review after non-trivial code edits and before commit or ship; also use for requested Codex, Claude, Pi, Droid, Copilot, Cursor, or OpenCode reviews."
---
...
Codex is the hard fallback when no CLI, environment, or repo-local config engine is set. This repo's current engine and provider default lives in `.agents/skills/autoreview/config.env`; keep provider-specific defaults there instead of duplicating them in this skill.
...
- The helper rejects assembled review prompts over 1,000,000 characters before invoking an engine. Treat this as a scope/transport problem: split the review target or keep bulky generated artifacts out of the diff and review their validator summaries and hashes. Do not reflexively disable the guard.
- Use `--max-bundle-chars N` or `AUTOREVIEW_MAX_BUNDLE_CHARS=N` only when the selected model's context is known to fit the complete prompt. `0` disables the guard and requires an intentional reason.
...
| Variable | Purpose |
|----------|---------|
| `AUTOREVIEW_ENGINE` | Override the default review engine |
...
| `AUTOREVIEW_MAX_BUNDLE_CHARS` | Maximum assembled review-prompt characters before engine invocation; defaults to `1000000`, and `0` disables the guard |

This repo also reads `.agents/skills/autoreview/config.env` after shell environment defaults. Explicit CLI flags still win over shell environment, and shell environment still wins over that repo-local config file.

OpenCode reviewers may load a provider credential from Doppler without writing it
to disk. Configure all four of
`AUTOREVIEW_OPENCODE_DOPPLER_PROJECT`,
`AUTOREVIEW_OPENCODE_DOPPLER_CONFIG`,
`AUTOREVIEW_OPENCODE_DOPPLER_SECRET`, and
`AUTOREVIEW_OPENCODE_DOPPLER_ENV`. The helper reads the secret with the machine
Doppler CLI identity, passes it only to the isolated OpenCode subprocess, and
never prints its value. An already-set target environment variable takes
precedence over Doppler.
...
Generated corpora, build outputs, fixtures, and per-item evidence can dominate a diff without improving model review. Keep deterministic validators, representative structure, manifest hashes, and summary reports in the review lane; review bulk artifacts separately or split them into their own target. A bundle-limit failure occurs before credentials or model tokens are consumed.
...
- supports `--engine codex`, `claude`, `droid`, `copilot`, `pi`, `opencode`, and `cursor`; default is explicit `--engine`, then `AUTOREVIEW_ENGINE`, then `.agents/skills/autoreview/config.env`, then `codex`
...
- enforces a default 1,000,000-character assembled-prompt limit before engine invocation, configurable with `--max-bundle-chars` / `AUTOREVIEW_MAX_BUNDLE_CHARS`
- identifies lifecycle-only zero-token engine output as a missing review result instead of misreporting valid JSONL events as malformed JSON
