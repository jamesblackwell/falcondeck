...
const qaPlanDetail = [
  "## Approach",
  "",
  "Entrypoint copies `bootstrap/studio-ui/` → `/workspace/ComfyUI/custom_nodes/comfyui-workstation-studio/` on boot (rsync/cp of the node dir). That is how the app reaches the volume; git alone does not. Same rule as the rest of `bootstrap/`.",
  "",
  "The node has no pip deps. Do not run `record-environment.sh` for this.",
  "",
  "## Screens",
  "",
  "### Generate (default)",
  "",
  "- Scrollable feed of this session's jobs, newest at the bottom above the dock (LucidPic create). Pending jobs show a progress placeholder via `/ws`.",
  "- Bottom dock always visible: textarea + generate. Settings chevron opens the sheet.",
  "- Tap a still → lightbox. Tap again / close to return.",
  "- Bottom nav: Generate | Library.",
  "",
  "### Library",
  "",
  "- Recursive listing of `ComfyUI/output/` (not just the folder root — subfolders per workflow).",
  "- Thumbnails lazy-load; the grid virtualizes past 200 items.",
  "",
  "## Files to touch",
  "",
  "1. `bootstrap/studio-ui/web/index.html` — shell + bottom nav.",
  "2. `bootstrap/studio-ui/web/app.js` — feed, dock, lightbox, `/ws` progress.",
  "3. `bootstrap/entrypoint.sh` — copy step (already generic; verify only).",
].join("\n");
