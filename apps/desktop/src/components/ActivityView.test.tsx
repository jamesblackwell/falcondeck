...
import { ActivityView } from '@falcondeck/chat-ui'
...
function props(overrides: Partial<React.ComponentProps<typeof ActivityView>> = {}) {
  return {
    groups: groups([]),
    interactiveRequests: [],
    onOpenThread: vi.fn(),
...
describe('ActivityView', () => {
  ...
  it('answers an approval inline with the exact response payload', async () => {
    ...
    await waitFor(() => expect(onInteractiveResponse).toHaveBeenCalledWith(
  ...
  it('shows inline response errors from the existing request card', async () => {
    render(<ActivityView {...props({
      groups: groups([thread({ id: 'blocked' })]),
      interactiveRequests: [request()],
      onInteractiveResponse: vi.fn().mockRejectedValue(new Error('Host rejected the response')),
    })} />)

    fireEvent.click(screen.getByRole('button', { name: 'Allow' }))
    expect(await screen.findByRole('alert')).toHaveTextContent('Host rejected the response')
  })
  ...
  it('disables remote-host writes while the host is offline', () => {
    ...
    render(<ActivityView {...props({
      ...
      interactiveRequests: [request()],
      workspaceHosts: { [workspace.id]: { name: 'Studio Mac', connected: false } },
      onInteractiveResponse,
      onMarkThreadRead,
    })} />)
