  ...
  return (
    <div className="flex flex-col gap-3 rounded-[var(--fd-radius-xl)] border border-border-subtle bg-surface-2 p-4 md:flex-row md:items-center md:justify-between">
      ...
      <div className="flex items-center gap-2">
        <span className="fd-type-readout truncate text-fg-muted">{device.device_id}</span>
        <Button
          type="button"
          variant="ghost"
          size="sm"
...
export function RemoteAccessPanel({
  ...
  return (
    <SettingsPage>
      <SettingsPageHeader
        title="Remote access"
        description="Pair devices, inspect relay health, and remove trusted devices from one place."
        actions={
      ...
      <SettingsSection
        ...
        description="Devices below can access this desktop session through the relay. Remove any device you no longer trust."
      ...
      <SettingsSection
        ...
        description="Quick health check for the desktop daemon and relay session."
        contentClassName="space-y-3 text-[length:var(--fd-text-sm)]"
      >
        <div className="flex items-center justify-between gap-4">
          <span className="text-fg-tertiary">Relay URL</span>
          <span className="truncate text-right text-fg-primary">
            {remoteStatus?.relay_url ?? relayUrl}
          </span>
        </div>
        <div className="flex items-center justify-between gap-4">
