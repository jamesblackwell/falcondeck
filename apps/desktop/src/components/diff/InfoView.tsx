...
export type ReviewInfoContext = {
  ...
  /** Casual folders hold chat documents but are intentionally not Git projects. */
  ...
  return (
    ...
    >
      <span
        ...
        style={{ width: `${(insertions / total) * 100}%` }}
      />
      <span
        className="bg-danger"
...
export const InfoView = memo(function InfoView({
  ...
  const isCasualChat = info.workspaceKind === "casual";

  return (
    <div className="pb-4 pt-3">
      {origin ? (
        <Row
          icon={<Waypoints aria-hidden="true" className="h-3.5 w-3.5" />}
          label="Origin"
        >
