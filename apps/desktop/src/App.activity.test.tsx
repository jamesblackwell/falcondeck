...
const testSnapshot = vi.hoisted(() => ({
  daemon: { version: '0.1.0', started_at: '2026-08-13T08:00:00Z', capabilities: {} },
  workspaces: [{
    id: 'workspace-1',
...
vi.mock('./hooks/useDaemonConnection', () => ({
  useDaemonConnection: () => {
    const [selectedWorkspaceId, setSelectedWorkspaceId] = useState<string | null>('workspace-1')
...
vi.mock('./components/DesktopShell', () => ({
  DesktopShell: ({ sidebar, main, rail }: { sidebar: React.ReactNode; main: React.ReactNode; rail?: React.ReactNode }) => (
    <div>
      <aside>{sidebar}</aside>
      <main>{main}</main>
...
describe('Activity takeover wiring', () => {
  it('suppresses the rail and closes the takeover when a thread is selected', async () => {
    ...
    fireEvent.click(within(screen.getByRole('main')).getByRole('button', { name: /Needs a decision/ }))
    await waitFor(() => {
