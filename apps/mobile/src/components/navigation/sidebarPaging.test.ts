...
it('keeps more available while a frozen page cursor remains, even after live inserts change counts', () => {
  const rows = buildSidebarRows([{ workspace: workspace(), threads: [thread()] }], new Set(), new Map(), null, 'last_updated', false, false, undefined,
...
it('makes unloaded casual chats accessible and removes more when scope is complete', () => {
  ...
  const rows = buildSidebarRows(groups, new Set(), new Map(), null, 'last_updated', true, false, undefined, counts)
  expect(rows.some(row => row.type === 'overflow' && row.workspaceId === 'workspace-1')).toBe(true)
  const complete = buildSidebarRows(groups, new Set(), new Map(), null, 'last_updated', true, false, undefined, counts, { 'workspace-1:last_updated': null })
