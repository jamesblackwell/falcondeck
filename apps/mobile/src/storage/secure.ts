...
const LEGACY_OPENROUTER_API_KEY = 'fd.openRouterApiKey'
...
/** Removes credentials written by the retired direct-from-phone transcription mode. */
export async function clearLegacyOpenRouterApiKey(): Promise<void> {
  try {
    await SecureStore.deleteItemAsync(LEGACY_OPENROUTER_API_KEY, STORE_OPTIONS)
  } catch (e) {
    console.warn('[SecureStore] clearLegacyOpenRouterApiKey failed', e)
  }
}
